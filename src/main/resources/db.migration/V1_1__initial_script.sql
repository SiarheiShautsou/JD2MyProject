...
create unique index if not exists l_training_type_id_uindex
    on training_place.l_trainings (training_type_id);

create unique index if not exists l_timetable_trainings_id_uindex
    on training_place.l_trainings (id);

create unique index if not exists l_timetable_trainings_trainer_id_training_date_uindex
    on training_place.l_trainings (trainer_id, training_date);

create unique index if not exists l_trainings_trainer_id_client_id_training_date_uindex
    on training_place.l_trainings (trainer_id, client_id, training_date);
...
(
    ...
    gym_id            integer                                   not null
        constraint l_gyms_trainers_gyms_id_fk
            references training_place.gyms (id)
            on update cascade on delete cascade,
    creation_date     timestamp(6) default CURRENT_TIMESTAMP(6) not null,
    modification_date timestamp(6) default CURRENT_TIMESTAMP(6) not null
);
...
create unique index if not exists roles_user_id_uindex
    on roles (user_id);
...
create table if not exists l_trainer_trainer_type
(
    id                integer      default nextval('l_trainer_trainer_type_column_1_seq'::regclass) not null,
    training_type_id  integer                                                                                      not null
        constraint l_trainer_trainer_type_training_types_id_fk
            references training_place.training_types (id)
            on update cascade on delete cascade,
    trainer_id        bigint                                                                                       not null
        constraint l_trainer_trainer_type_users_id_fk
            references training_place.users (id)
            on update cascade on delete cascade,
    creation_date     timestamp(6) default CURRENT_TIMESTAMP(6)                                                    not null,
    modification_date timestamp(6) default CURRENT_TIMESTAMP(6)                                                    not null
...
alter table l_trainer_trainer_type
    owner to postgres;

create unique index if not exists l_trainer_trainer_type_column_1_uindex
    on l_trainer_trainer_type (id);
